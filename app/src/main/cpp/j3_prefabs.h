...
void j3_prefabs_init(void);
void j3_prefabs_shutdown(void);
 ...
 * four. */
void j3_prefabs_add(const char* id, const char* name,
                    const char* category, const char* tags,
                    const char* glyph, uint32_t param_mask);

void j3_prefabs_clear(void);
int  j3_prefabs_count(void);

void j3_prefabs_set_spawn_callback(J3SpawnFn fn, void* user);
void j3_prefabs_set_scan_callback(J3ScanFn fn, void* user);
 ...
 * current context. */
void j3_prefabs_draw(void);

/* For hosts that already have their own window/tab: draws just the body with no
 * Begin/End of its own. */
void j3_prefabs_draw_body(void);

/* ---- telemetry (for the host, optional) ------------------------------- */

int         j3_prefabs_total_spawned(void);
const char* j3_prefabs_last_spawn(void);   /* prefab id, or "" */
